import type { Model, ModelVariant } from "@/generated/prisma";
...
export const GET = async (req: NextRequest) => {
	...
	try {
		...
		const lvMakes = makes.map(({ name, id }) => ({
			label: name,
			value: id.toString(),
		}));

		const lvModels = models.map(({ name, id }) => ({
			label: name,
			value: id.toString(),
		}));

		const lvModelVariants = modelVariants.map(({ name, id }) => ({
			label: name,
			value: id.toString(),
		}));
